...
export default function Feed() {
  ...
  const getPlaceholderImage = (prompt: string, id: string) => {
    ...
    const seed = parseInt(id, 10) || Math.random() * 1000;

    // Map keywords to Picsum categories with specific image IDs for consistency
    if (keywords.includes("sunset") || keywords.includes("mountain")) {
      return `https://picsum.photos/400/400?random=${seed + 1}`;
    }
    if (keywords.includes("city") || keywords.includes("futuristic")) {
      return `https://picsum.photos/400/400?random=${seed + 2}`;
    }
    if (keywords.includes("cat") || keywords.includes("animal")) {
      return `https://picsum.photos/400/400?random=${seed + 3}`;
    }
    if (keywords.includes("forest") || keywords.includes("nature")) {
      return `https://picsum.photos/400/400?random=${seed + 4}`;
    }
    if (keywords.includes("ocean") || keywords.includes("water")) {
      return `https://picsum.photos/400/400?random=${seed + 5}`;
    }
    if (keywords.includes("space") || keywords.includes("cosmic")) {
      return `https://picsum.photos/400/400?random=${seed + 6}`;
    }
    // Default fallback
