...
export default function Home() {
  ...
  // Function to generate more appropriate placeholder images
  const getPlaceholderImage = (prompt: string, id: string) => {
    const keywords = prompt.toLowerCase();

    // Map keywords to Unsplash search terms
    if (keywords.includes("sunset") || keywords.includes("mountain")) {
      return `https://source.unsplash.com/400x400/?sunset,mountains&sig=${id}`;
    }
    if (keywords.includes("city") || keywords.includes("futuristic")) {
      return `https://source.unsplash.com/400x400/?city,futuristic&sig=${id}`;
    }
    if (keywords.includes("cat") || keywords.includes("animal")) {
      return `https://source.unsplash.com/400x400/?cat,cute&sig=${id}`;
    }
    if (keywords.includes("forest") || keywords.includes("nature")) {
      return `https://source.unsplash.com/400x400/?forest,nature&sig=${id}`;
    }
    if (keywords.includes("ocean") || keywords.includes("water")) {
      return `https://source.unsplash.com/400x400/?ocean,water&sig=${id}`;
    }
    if (keywords.includes("space") || keywords.includes("cosmic")) {
      return `https://source.unsplash.com/400x400/?space,galaxy&sig=${id}`;
    }
    // Default fallback
    return `https://source.unsplash.com/400x400/?abstract,art&sig=${id}`;
  };
        ...
        if (data.success) {
          const recommendedImages: GeneratedImage[] = data.recommendations.map(
            (img: {
              ...
              id: img.id,
              url: getPlaceholderImage(img.prompt, img.id), // Better matching images
              prompt: img.prompt,
              likes: img.likes,
              comments: [],
